...
import { Accessor, COMP_SCOPE, FRAG_SCOPE, Fragment, SVG_SCOPE, createMemo, createState, h, popScope, pushScope } from "./deps.ts"
...
export const Clock: ComponentGenerator = () => {
	const [idTime, getTime, setTime] = createState<number>(getSecondsSinceMidnight())
	const dispose = setInterval(() => {
		setTime(getSecondsSinceMidnight())
	}, 50)
	// ctx.onDelete(idTime, () => clearInterval(dispose))

	const rotate = (rotate: number) => `rotate(${(rotate * 360) | 0})`

	const [, getSubsecond] = createMemo((id) => rotate(getTime(id) % 1), { defer: false })
	...
	const [, getHour] = createMemo((id) => rotate(((getTime(id) / 60 / 60) % 12) / 12), { defer: false })

	globalThis.getSecond = getSecond
